...
export default function Chatcontainer({ currentChatUser }) {
    ...
    useEffect(()=>{
        const getmessages = async()=>{   
          try{
            const response = await axios.get(`http://localhost:4000/api/post/get/chat/msg/${id}/${currentChatUser.others._id}`,{
              headers:{
    ...
    console.log(message)
    //   const sendmsg =()=>{
    //     fetch(`http://localhost:4000/api/post/msg`,{method:'POST',
    // headers:{
    ...
    const sendmsg = () => {
        ...
        fetch(`http://localhost:4000/api/post/msg`, {
            method: 'POST',
            headers: {
