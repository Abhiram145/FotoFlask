...
export default function Post(props) {
  ...
  useEffect(() => {
    const getuserdetails = async () => {
      try {
        const details = await axios.get(`http://localhost:4000/api/user/post/user/details/${props.post.user}`,{
          headers:{
            jwttoken:jwt_here
          }
        });
  ...
  const addComment = async () => {
    ...
    await fetch(`http://localhost:4000/api/post/comment/post`,
      {
        method: "PUT",
        headers: {
  ...
  const handleLike = async () => {
    if (Like === LikeIcon) {
      ...
      await fetch(`http://localhost:4000/api/post/${props.post._id}/like`,
        {
          method: "PUT",
          headers: {
            'Content-Type': 'application/json',
