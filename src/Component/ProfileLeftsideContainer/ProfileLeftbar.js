...
export default function ProfileLeftbar({profileid}) {
  ...
  useEffect(()=>{
    const getuserdetails =async()=>{
      try{
      const details = await axios.get(`http://localhost:4000/api/user/post/user/details/${profileid}`,{
        headers:{
          jwttoken:jwt_here
        }
      });
    ...
    const getfollowings=async()=>{
      try{   
          ...
          const response = await axios.get(`http://localhost:4000/api/user/get/followings/${profileid}`,{
            headers:{
              jwttoken:jwt_here
            }
          })
...
//   await fetch(`http://localhost:4000/api/user/follow/${id}`,{method:"PUT" ,
//   headers:{
...
const handleFollow = async()=>{
  if(Follow === "Follow"){
    await fetch(`http://localhost:4000/api/user/follow/${profileid}` , {method:'PUT', headers:{'Content-Type':"application/JSON" , jwttoken:jwt_here} , body:JSON.stringify({user:`${user.user._id}`})})
